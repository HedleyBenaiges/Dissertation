...
def attack_basemul_pair(pair_idx: int,
    ...
    window_len = max(bm_len // n_pairs, 1)
    w_start    = bm_start + pair_idx * window_len
    w_end      = min(w_start + window_len, bm_end)

    trc_f = traces.astype(np.float32)
    trc_c = trc_f - trc_f.mean(axis=0)
    trc_var = (trc_c**2).sum(axis=0) + 1e-10

    # Get the window's data
    trc_c_win   = trc_c[:, w_start:w_end]
    trc_var_win = trc_var[w_start:w_end]

    # Within the window, pick top 20% of cycles by variance as POI
    if len(trc_var_win) > 0:
        poi_thresh = np.percentile(trc_var_win, 80)
        poi_local  = np.where(trc_var_win >= poi_thresh)[0]
        if len(poi_local) == 0:
            poi_local = np.arange(len(trc_var_win))
    else:
        poi_local = np.array([0])
