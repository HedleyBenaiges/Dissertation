...
int main(void) {
    ...
    /* ---- FIXED set ---- */
    uint8_t b0, b1, b2;
    randbyte(&b0); randbyte(&b1); randbyte(&b2);
    val = (int32_t)(((uint32_t)b2 << 16) | ((uint32_t)b1 << 8) | (uint32_t)b0);
    for (i = 0; i < NOTRACES; i++) {
        // We do exactly ONE reduction per trace to keep the simulator clean
        starttrigger();
        out_buf[0] = PQCLEAN_MLKEM512_CLEAN_montgomery_reduce(val);
        endtrigger();
    }

    /* ---- RANDOM set ---- */
    for (i = 0; i < NOTRACES; i++) {
        // Fill one random input outside the trigger
        uint8_t b0, b1, b2;
        randbyte(&b0); randbyte(&b1); randbyte(&b2);
        val = (int32_t)(((uint32_t)b2 << 16) | ((uint32_t)b1 << 8) | (uint32_t)b0);
