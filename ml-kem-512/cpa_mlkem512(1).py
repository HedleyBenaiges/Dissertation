...
Usage:
  ...
  --num-traces    Number of traces to use    (default: 500)
...
Notes:
  ...
  - With 500 traces this script recovers 256 NTT-domain coefficients of
...
def load_randdata(randdata_path: str,
    ...
    with open(randdata_path) as f:
        raw = [int(line.strip(), 16) for line in f if line.strip()] # TODO: fix here
...
def attack_basemul_pair(pair_idx: int,
    ...
    # Restrict correlation search to high-variance cycles to save time.
    # Use top 10% of cycles by variance as points of interest (POI).
    poi_thresh = np.percentile(trc_var, 95)
    poi        = np.where(trc_var >= poi_thresh)[0]
    trc_c_poi  = trc_c[:, poi]                  # (N, |POI|)
    trc_var_poi = trc_var[poi]
...
def main():
    ...
    parser.add_argument('--num-traces',  type=int,  default=500)
