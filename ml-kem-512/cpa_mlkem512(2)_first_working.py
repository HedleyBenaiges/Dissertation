...
Usage:
  ...
  --asmtrace      Path to asmtrace for POI   (default: auto-detect via variance)
...
def poly_frombytes(a: bytes) -> np.ndarray:
    ...
    for i in range(KYBER_N // 2):
        ...
        # TODO: previously: r[2*i+1] = np.int16((a[3*i+1] >> 4) | (a[3*i+2] << 4) & 0xfff)
    return r
...
def attack_basemul_pair(pair_idx: int,
    ...
    polyvec_basemul_acc_montgomery calls poly_basemul_montgomery, which
    iterates: for i in 0..63:
        basemul(r[4i:4i+2],   a[4i:4i+2],   b[4i:4i+2],   zetas[64+i])
        basemul(r[4i+2:4i+4], a[4i+2:4i+4], b[4i+2:4i+4], -zetas[64+i])
    
    pair_idx 0..63  → first basemul  call (zeta = +zetas[64+pair_idx//2])
    pair_idx 64..127 → second basemul call (zeta = -zetas[64+(pair_idx-64)//2])

    Actually simpler: for j in 0..127 (overall pair index):
        loop_i = j // 2
        zeta = zetas[64 + loop_i] if j % 2 == 0 else -zetas[64 + loop_i]
        base coeffs: 4*loop_i + 2*(j%2)  (i.e. 4*loop_i or 4*loop_i+2)

    Args:
      pair_idx: 0..127, which 2-coeff basemul
      vec_idx:  0 or 1, which polynomial of the polyvec to attack
      a_ntt:    (N, K, 256) int16 NTT-domain ciphertext coefficients
      traces:   (N, T) float32 power traces
      zetas:    (128,) int16 from ntt.c

    Returns:
      (best_s0, best_s1): recovered int16 secret NTT coefficients
      max_corr:           peak absolute Pearson correlation
    """
    loop_i   = pair_idx // 2
    second   = (pair_idx % 2 == 1)
    ...
    # Precompute per-trace contributions split by s0 and s1 independently
    # to avoid holding the full (C, C, N) array in memory.
    #
    # For each s0 candidate: contrib_s0[i] = fqmul(a0[trace], s0) → (N,) int16
    # For each s1 candidate: contrib_s1[j] = fqmul(fqmul(a1[trace], s1), zeta) → (N,) int16
    # r0[i,j,trace] = contrib_s0[i,trace] + contrib_s1[j,trace]
    #
    # We iterate over s0 in an outer loop and vectorise over s1 to keep
    # memory at O(C × N) per iteration.
    ...
    # Precompute trace std for correlation: (T,)
    T       = traces.shape[1]
    trc_f   = traces.astype(np.float32)
    trc_mu  = trc_f.mean(axis=0)
    trc_c   = trc_f - trc_mu
    trc_var = (trc_c**2).sum(axis=0) + 1e-10

    # Restrict correlation to the time window for this basemul pair.
    # There are KYBER_N // 2 = 128 basemul calls evenly spaced in the trace,
    # so divide the trace into 128 equal-width windows and pick this pair's
    # window.  Within the window a tight variance percentile gives only the
    # most informative cycles, keeping the inner loop fast while avoiding the
    # random noise spikes that dominate a flat high-percentile over the whole
    # trace.
    n_pairs    = KYBER_N // 2                        # 128
    window_len = max(T // n_pairs, 1)
    t_start    = pair_idx * window_len
    t_end      = min(t_start + window_len, T)

    trc_c_win   = trc_c[:, t_start:t_end]           # (N, window_len)
    trc_var_win = trc_var[t_start:t_end]             # (window_len,)

    # Within the window pick the top 1% of cycles by variance.
    # This is a small, stable set that sits squarely inside the basemul
    # execution, so it is both fast and meaningful.
    poi_thresh  = np.percentile(trc_var_win, 90)
    poi_local   = np.where(trc_var_win >= poi_thresh)[0]
    if len(poi_local) == 0:                          # safety fallback
        poi_local = np.arange(len(trc_var_win))

    trc_c_poi   = trc_c_win[:, poi_local]            # (N, |POI|)
    trc_var_poi = trc_var_win[poi_local]             # (|POI|,)

    for s0 in cand_range:
        # Contribution of a0 * s0: (N,) int16
        c_s0 = montgomery_reduce_vec(
            a0.astype(np.int32) * np.int32(int(s0))
        )  # (N,)

        # r0 = c_s0[None, :] + contrib_s1_all: (C, N) int16
        r0_all = (c_s0.astype(np.int32)[None, :] +
                  contrib_s1_all.astype(np.int32)).astype(np.int16)  # (C, N)

        # Hamming weight: (C, N) uint8
        hw = hamming_weight_vec(r0_all)        # (C, N)

        # Pearson correlation with traces at POI: (C, |POI|)
        hw_f  = hw.astype(np.float32)
        hw_mu = hw_f.mean(axis=1, keepdims=True)
        hw_c  = hw_f - hw_mu
        hw_sd = np.sqrt((hw_c**2).sum(axis=1, keepdims=True)) + 1e-10
        hw_n  = hw_c / hw_sd                   # (C, N) normalised

        # corr[s1_idx] = max over POI of |Pearson(hw[s1_idx,:], trace[:,t])|
        corr_poi = (hw_n @ trc_c_poi) / (np.sqrt(trc_var_poi)[None, :])
        # (C, |POI|) — take max absolute correlation over POI for each s1
        max_corr_per_s1 = np.abs(corr_poi).max(axis=1)  # (C,)
...
def main():
    ...
    print(f"\n[4/5] Decompressing and computing NTT of ciphertexts ...")
    t0 = time.time()
    ...
    # -- CPA main loop -----------------------------------------------------
    print(f"\n[5/5] CPA over basemul pairs {args.start_pair}–{args.end_pair-1}"
          f"  (vec_idx={args.vec_idx})")
    print(f"      Each pair: q²={KYBER_Q**2:,} candidates  |  "
          f"estimated ~5 min/pair")
    print()
